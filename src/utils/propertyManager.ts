class PropertyManager {
	...
	static isDefined = (value: any) => value !== null && typeof value !== 'undefined';

	static hasProperty = (obj: any, propertyName: string) => {
		if (!PropertyManager.isDefined(obj) || !PropertyManager.isDefined(propertyName)) {
			return false;
		}
		...
		if (postDot > 0 && postDot < propertyName.length - 1) {
			return Object.prototype.hasOwnProperty.call(obj, propertyName.substring(0, postDot));
		}
	...
	static getValue(model: any, propertyName: any): any {
		...
		if (postDot > 0 && postDot < propertyName.length - 1) {
			...
			const propSecondBlock = propertyName.substring(postDot + 1, propertyName.length);
